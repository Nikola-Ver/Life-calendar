...
const Tuning: React.FC<TuningProps> = (props) => {
  function addNewTasks(): void {
    const titleDiv = document.getElementsByClassName(
      'Cell-tuning-fields-title'
    )[0] as any;
    const textDiv = document.getElementsByClassName(
      'Cell-tuning-fields-text'
    )[0] as any;
    ...
    elementRemove.className =
      'Editor-delete Calendar-cell-info-content-element-remove';
    elementRemove.onclick = (e: any) => {
  ...
  function changeMood(e: any): void {
    ...
    for (let i = 0; i < moodDivs.length; i++) {
      if (moodDivs[i].classList.contains('active'))
        moodDivs[i].classList.remove('active');
    }
  ...
  return (
    <div className='Cell-tuning'>
      <div className='Cell-tuning-fields'>
        <TextareaAutosize
          className='Cell-tuning-fields-title'
          placeholder='Title'
        />
        <TextareaAutosize
          className='Cell-tuning-fields-text'
          placeholder='Completed tasks'
        />
        <div
          className='Cell-tuning-fields-button'
          onClick={() => {
            addNewTasks();
          }}
        >
